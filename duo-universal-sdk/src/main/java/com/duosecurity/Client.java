...
import static com.duosecurity.Validator.validateRedirectUri;
...
public class Client {
	...
	// **************************************************
	// Constructors
	// This class uses the "Builder" pattern and should not be directly
	// instantiated.
...
//	 */
	@Deprecated
	public Client(String clientId, String clientSecret, String apiHost) throws DuoException {
		this(clientId, clientSecret, apiHost, null, null, null);
	}

	@Deprecated
	public Client(String clientId, String clientSecret, String apiHost, String[] certs)
			throws DuoException {
		this(clientId, clientSecret, apiHost, null, null, certs);
	}
	 ...
	 */
	@Deprecated
	public Client(String clientId, String clientSecret, String apiHost, String proxyHost, Integer proxyPort,
			String[] userCaCerts) throws DuoException {
		Client client = new Builder(clientId, clientSecret, proxyHost, proxyPort, apiHost).setCACerts(userCaCerts)
				.build();
	...
	public static class Builder {
      ...
      //C=US, O=DigiCert Inc, OU=www.digicert.com, CN=DigiCert Assured ID Root CA
		...
		public Builder(String clientId, String clientSecret, String apiHost) {
			this(clientId, clientSecret, null, null, apiHost);
		}
		 ...
		 *                     2FA is completed.
		 */
		public Builder(String clientId, String clientSecret, String proxyHost, Integer proxyPort, String apiHost) {
		...
		public Client build() throws DuoException {
			validateClientParams(clientId, clientSecret, apiHost);
		...
		/**
		 * Optionally toggle the returned authorization parameter to use duo_code vs
		 * code. Defaults true to use duo_code.
		 *
		 ...
		 *
		 * @param newUserAgent Additional info that will be added to the end of the user
		 *                     agent string
		 * 
		 * @return the Builder
		 */
		...
		private String computeUserAgent() {
			...
			String javaAgent = format("%s/%s", System.getProperty("java.vendor"), System.getProperty("java.version"));
			String osAgent = format("%s/%s/%s", System.getProperty("os.name"), System.getProperty("os.version"),
	...
	public HealthCheckResponse healthCheck() throws DuoException {
		...
		HealthCheckResponse response = duoConnector.duoHealthcheck(clientId, createJwt(clientId, clientSecret, aud));
	...
	/**
	 * Constructs a string which can be used to redirect the client browser to Duo
	 * for 2FA.
	 *
	 * @param username The user to be authenticated by Duo.
	 * @param state    A randomly generated String with at least 22 characters This
	 *                 value will be returned to the integration post 2FA and should
	 *                 be validated. {@link #generateState} exists as a utility
	 *                 function to generate this param.
	 * @return String
	 *
	 * @throws DuoException For problems creating the auth url
	 */
	public String createAuthUrl(String username, String state, String redirectUri) throws DuoException {
		validateUsername(username);
		validateState(state);
		validateRedirectUri(redirectUri);
		String request = createJwtForAuthUrl(clientId, clientSecret, redirectUri, state, username, useDuoCodeAttribute);
		String query = format("?scope=openid&response_type=code&redirect_uri=%s&client_id=%s&request=%s", redirectUri,
				clientId, request);
	...
	/**
	 * Verifies the duoCode returned by Duo and exchanges it for a {@link Token}
	 * which contains information pertaining to the auth. Uses the default token
	 * validator defined in DuoIdTokenValidator. To use a custom validator, see
	 * exchangeAuthorizationCodeFor2FAResult(String, TokenValidator)
	 *
	 * @param duoCode  This string is an identifier for the auth and should be
	 *                 exchanged with Duo for a token to determine if the auth was
	 *                 successful as well as obtain meta-data about about the auth.
	 *
	 * @param username The user to be authenticated by Duo
	 *
	 * @return {@link Token}
	 ...
	 */
	public Token exchangeAuthorizationCodeFor2FAResult(String duoCode, String username, String redirectUri)
			throws DuoException {
		TokenValidator validator = new DuoIdTokenValidator(clientSecret, username, clientId, apiHost);
		return exchangeAuthorizationCodeFor2FAResult(duoCode, validator, redirectUri);
	}

	/**
	 * Verifies the duoCode returned by Duo and exchanges it for a {@link Token}
	 * which contains information pertaining to the auth. This version of the method
	 * allows the use of a custom JWT token validator. If you use a custom
	 * validator: You MUST confirm the integrity of the token by using the client
	 * secret to validate the SHA512 HMAC You MUST check the claims for Issuer
	 * Audience Issued at / Expiration Username (preferred_username)
	 *
	 * @param duoCode   This string is an identifier for the auth and should be
	 *                  exchanged with Duo for a token to determine if the auth was
	 *                  successful as well as obtain meta-data about the auth.
	 *
	 * @param validator A TokenValidator that will validate and decode the JWT ID
	 *                  Token provided by Duo.
	 *
	 * @return {@link Token}
	 *
	 * @throws DuoException For errors exchanging duoCode for 2FA results
	 *
	 */
	public Token exchangeAuthorizationCodeFor2FAResult(String duoCode, TokenValidator validator, String redirectUri)
			throws DuoException {
		String aud = getAndValidateUrl(apiHost, OAUTH_V_1_TOKEN_ENDPOINT).toString();
		TokenResponse response = duoConnector.exchangeAuthorizationCodeFor2FAResult(userAgent, "authorization_code",
				duoCode, redirectUri, CLIENT_ASSERTION_TYPE, createJwt(clientId, clientSecret, aud));
	...
	/**
	 * Generates a 36 character random identifier to be used as the state variable
	 * in the createAuthUrl method. This value should be stored in a variable and
	 * validated against the state returned by Duo.
	 *
	 * @return String
	 */
