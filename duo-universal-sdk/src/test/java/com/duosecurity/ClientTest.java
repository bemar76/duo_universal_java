...
class ClientTest {
	...
	void setUp() throws DuoException {
		this.client = new Client.Builder(CLIENT_ID, CLIENT_SECRET, null, null, API_HOST).build();
	...
	void createAuthUrl_success() throws DuoException {
		String urlString = client.createAuthUrl(USERNAME, STATE, HTTPS_REDIRECT_URI);
		try {
	...
	void createAuthUrl_throws_exception_for_invalid_username() {
		try {
			client.createAuthUrl("", STATE, HTTPS_REDIRECT_URI);
			Assertions.fail();
		} catch (DuoException e) {
	...
	void createAuthUrl_throws_exception_for_invalid_state() {
		try {
			client.createAuthUrl(USERNAME, "", HTTPS_REDIRECT_URI);
			Assertions.fail();
		} catch (DuoException e) {
	...
	void createAuthUrl_throws_exception_for_invalid_client_id() throws DuoException {
		try {
			Client badClient = new Client.Builder("", CLIENT_SECRET, API_HOST).build();
			badClient.createAuthUrl(USERNAME, STATE, HTTPS_REDIRECT_URI);
			Assertions.fail();
		} catch (DuoException e) {
	...
	void createAuthUrl_throws_exception_for_invalid_client_secret() throws DuoException {
		try {
			Client badClient = new Client.Builder(CLIENT_ID, "", API_HOST).build();
			badClient.createAuthUrl(USERNAME, STATE, HTTPS_REDIRECT_URI);
			Assertions.fail();
		} catch (DuoException e) {
	...
	void createAuthUrl_throws_exception_for_invalid_api_host() throws DuoException {
		try {
			Client badClient = new Client.Builder(CLIENT_ID, CLIENT_SECRET, "").build();
	...
	void exchangeAuthorizationCodeFor2FAResult_success() throws DuoException {
		...
		tokenResponse.setId_token(
				"eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiYWRtaW4iOnRydWUsImlhdCI6MTUxNjIzOTAyMn0.qKLZNpctaGsuqLr6KkPiM7_9jG5sEEaLPLakrA1kjk7z0lF3HX_RTRS3c4wVFWMEV_jGg72KIjlBpsWrqMxSNg");
		Mockito.when(client.duoConnector.exchangeAuthorizationCodeFor2FAResult(anyString(), anyString(), anyString(),
				anyString(), anyString(), anyString())).thenReturn(tokenResponse);
		...
		Token result = client.exchangeAuthorizationCodeFor2FAResult("duo_code", stubValidator, HTTPS_REDIRECT_URI);
	...
	void exchangeAuthorizationCodeFor2FAResult_throws_exception_for_invalid_api_host() throws DuoException {
		try {
			Client badClient = new Client.Builder(CLIENT_ID, CLIENT_SECRET, "").build();
			badClient.exchangeAuthorizationCodeFor2FAResult("duo_code", "username", HTTPS_REDIRECT_URI);
			Assertions.fail();
		} catch (DuoException e) {
			assertTrue(e.getMessage().contains("Invalid host"));
		}
	...
	void useDuoCodeAttribute_defaults_true() throws DuoException {
		// Don't rely on setUp Client just in case that ever changes to longform
		// constructor
		Client client = new Client.Builder(CLIENT_ID, CLIENT_SECRET, API_HOST).build();
		String urlString = client.createAuthUrl(USERNAME, STATE, HTTPS_REDIRECT_URI);
	...
	void useDuoCodeAttribute_set_false() throws DuoException {
		Client client = new Client.Builder(CLIENT_ID, CLIENT_SECRET, API_HOST).setUseDuoCodeAttribute(false).build();
		String urlString = client.createAuthUrl(USERNAME, STATE, HTTPS_REDIRECT_URI);
		HttpUrl url = HttpUrl.parse(urlString);
		String jwtString = url.queryParameter("request");
		DecodedJWT jwt = JWT.decode(jwtString);
	...
	void custom_useragent() throws DuoException {
		...
		Client client = new Client.Builder(CLIENT_ID, CLIENT_SECRET, API_HOST).appendUserAgentInfo(appendedUserAgent)
				.build();
		...
		verify(client.duoConnector).exchangeAuthorizationCodeFor2FAResult(stringCaptor.capture(), anyString(),
	...
	void legacy_constructors_match() throws DuoException {
		// Create clients using the old deprecated constructors and check that their
		// fields are the same as one created using the builder.
		// This should help prevent adding a new field to the class and forgetting to
		// update the legacy constructors.
		// Unfortunately duoConnector is an object entity that can't be compared.

		Client builderClient = new Client.Builder(CLIENT_ID, CLIENT_SECRET, API_HOST).build();
		Client shortConstructorClient = new Client(CLIENT_ID, CLIENT_SECRET, API_HOST);
		Client longConstructorClient = new Client(CLIENT_ID, CLIENT_SECRET, API_HOST, null);
